...
shot_noise_tfa <-
  ...
  orbi_filter_scan_intensity(outlier_percent = 2.1) |>
...
```{r}
#| label: fig-15-shotnoise-tfa-ITs
...
```{r}
#| label: fig-15-shotnoise-tfa-resolution
...
shot_noise_aas <-
  ...
  # implement: orbi_flag_satellite_peaks() |>
  # implement: orbi_flag_weak_isotopocules() |>
  
  orbi_filter_satellite_peaks() |>
  orbi_filter_weak_isotopocules(min_percent = 10) |>
  
  
  # switch to: orbi_flag_outliers(intensity_window = c(2, 98))
  # deprecate: orbi_filter_scan_intensity(outlier_percent = 2.1) |>
  
  
  # implement: orbi_remove_flagged_data() |>
  #dplyr::filter(!satellite, !weak, !outlier) |>
...
shot_noise_aas |>
  ...
  # x-axis parameter --> allow flexibility with this
  orbi_plot_shot_noise()
```


# Figure 19: FIXME continue here

```{r, eval = FALSE}
data <- 
  "data/mrfa/replicates/data" |>
  orbi_find_isox() |>
  orbi_read_isox()
