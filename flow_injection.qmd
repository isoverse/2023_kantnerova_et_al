...
flow_injections <- 
  ...
  orbi_find_isox() |>
  orbi_read_isox() |>
  # get injection number, and sample name from filename and add tuning to filename
  mutate(
    tuning = filepath |> dirname() |> basename() |> str_extract("bad|optimal"),
    injection = filename |> str_extract("\\d+$") |> as.integer(),
    sample_name = ifelse(injection %% 2 == 0, "USGS35", "USGS32"),
    filename = paste(tuning, filename),
    .after = 1L
  ) |>
  orbi_simplify_isox(add = c("intensity", "tuning", "injection", "sample_name")) |>
...
flow_injections_summary <- 
  ...
  mutate(
    tuning = filename |> str_extract("bad|optimal"),
    is_ref = sample_name == "USGS35"
  )
```
...
fig_a <- base_plot %+%
  filter(flow_injections, tuning == "bad", isotopocule == "M0") %+%
  aes(y = intensity) +
  scale_y_continuous(
...
fig_b <- 
  base_plot %+% 
  filter(flow_injections_ratios, tuning == "bad", isotopocule == "18O") %+% 
  aes(y = ratio)
fig_b
```
...
fig_c <- 
  flow_injections_summary |>
  filter(data_type == "data", tuning == "bad", isotopocule == "18O") |>
  # plot
  ggplot() +
  aes(
    x = injection,
...
fig_d <- fig_c %+%
  ...
  labs(y = expression(delta^18*O~"(USGS32 vs. VSMOW) in permil"))
fig_d
```
